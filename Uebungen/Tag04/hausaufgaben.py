from docx import Document
from reportlab.lib.pagesizes import A4
from reportlab.pdfgen import canvas
#Nicht vergessen, die benötigten Pakete zu installieren:
#pip install python-docx reportlab

class Dateikonverter:
    """Konvertiert eine Textdatei in verschiedene Formate: Markdown, Word, PDF."""
    def __init__(self, input_file):
        self.input_file = input_file
    """Liest den Inhalt der Eingabedatei ein."""
    def _read_text(self):
        with open(self.input_file, "r", encoding="utf-8") as f:
            return f.read()
    """Konvertiert die Eingabedatei in eine Markdown-Datei."""
    def to_markdown(self, output_file):
        text = self._read_text()
        with open(output_file, "w", encoding="utf-8") as f:
            f.write(f"# Converted File\n\n{text}")
    """Konvertiert die Eingabedatei in eine Word-Datei."""
    def to_word(self, output_file):
        text = self._read_text()
        doc = Document()
        doc.add_heading("Converted File", level=1)
        doc.add_paragraph(text)
        doc.save(output_file)
    """Konvertiert die Eingabedatei in eine PDF-Datei."""
    def to_pdf(self, output_file):
        text = self._read_text()
        c = canvas.Canvas(output_file, pagesize=A4)
        width, height = A4
        y = height - 50
        for line in text.splitlines():
            c.drawString(50, y, line)
            y -= 15
            if y < 50:
                c.showPage()
                y = height - 50
        c.save()
    """Beispielaufrufe der Konvertierungsmethoden."""
test_konvertierer = Dateikonverter("zuKonvertierendeDatei.txt") 
test_konvertierer.to_pdf("konvertierteDatei.pdf")