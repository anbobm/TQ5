...
class Bibliothek:
    ...
    def importieren(self):
        with open("bücher.txt", encoding="utf-8", newline='') as file:
            reader = csv.reader(file)
            for zeile in reader:
                titel, erscheinungsjahr, autor, seitenzahl = zeile
                buch = Buch(titel, int(erscheinungsjahr), autor, int(seitenzahl))
                self.hinzufuegen(buch)

        with open("dvds.csv", encoding="utf-8", newline='') as file:
            reader = csv.reader(file)
            for zeile in reader:
                titel, erscheinungsjahr, spielzeit, regisseur = zeile
